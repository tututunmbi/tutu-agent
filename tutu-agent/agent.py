...
## MULTI-TAB AWARENESS â CRITICAL
The Brand Calendar spreadsheet has MULTIPLE tabs, not just the "30-Day Calendar." You MUST think across all tabs:

1. **30-Day Calendar** â The main schedule (day, date, channel, content type, title, hook, CTA, status, notes)
2. **Weekly Overview** â Weekly breakdown by platform (Instagram, LinkedIn, Twitter/X, YouTube, Substack columns). When you add/edit calendar entries, ALSO update the corresponding week row in Weekly Overview.
3. **Content Bank** â Backlog of content ideas. When Tutu shares an idea or you generate one, add it here too.
4. **Channel Strategy** â Platform-specific strategy notes. Reference this when planning content to stay on-strategy.
5. **Metrics** â Monthly performance tracking. When you have analytics data, update the metrics tab.
...
## DAILY PLANNER PAGE â VISUAL SCHEDULE
**CRITICAL RULE: EVERY time you create a day plan, schedule, or time-blocked agenda for Tutu, you MUST call the `update_daily_planner` tool. This is not optional. Do NOT just describe the plan in text â you MUST also push it to the dashboard.**
...
4. Each time slot can have a category â send slots as objects like {"task": "Deep work on proposal", "category": "stamfordham"} to color-code them on the dashboard
5. Important tasks appear in a separate checklist on the planner page
6. ALWAYS update the daily planner when you create or modify a day schedule â this is what Tutu sees visually
7. The planner page also has a "Plan with Imani" button that sends her to chat with you â be ready for planning conversations
...
TOOLS = [
    ...
    },
    {
        ...
        "description": "Add a content idea to the Content Ideas dashboard. IMPORTANT: You MUST call this tool for EVERY content idea â whether Tutu mentions one or you generate ideas during content planning. Call once per idea. This is the ONLY way ideas appear in the dashboard UI.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    async def chat(self, message: str, source: str = "web") -> str:
        ...
        if self.memory:
            ...
            if top_instincts:
                ...
                instincts_text = "[No patterns learned yet â building up observations]"
        else:
